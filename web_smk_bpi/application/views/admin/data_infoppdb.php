	...
	<div class="col-md-9">
		<div style="width:100%; float:left; border:1px solid;">
			<div class="col-xs-12">
				...
				<?php
					$i=$j;
					foreach ($list_infoppdb as $list){
				?>
					<tr>
						<td><?php echo $i; ?></td>
						<td><?php echo $list->judul_info_ppdb; ?></td>
						<td><?php $text=$list->info_ppdb; $text=character_limiter($text, 15); echo $text;?></td>
						<td><?php echo $list->tgl_posting; ?></td>
						<td><?php echo $list->nama_user; ?></td>
						<td><a data-toggle="modal" href="#detail<?php echo $list->id_info_ppdb; ?>" class="btn btn-info btn-xs" title="lihat"><span class="glyphicon glyphicon-eye-open"></span></a> <a data-toggle="modal" href="#hapus<?php echo $list->id_info_ppdb; ?>" data-dismiss="modal" class="btn btn-info btn-xs" title="Hapus"><span class="glyphicon glyphicon-trash"></span></a></td>
					</tr>
					
					<div class="modal fade" id="detail<?php echo $list->id_info_ppdb; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-body">
									<?php echo $list->info_ppdb; ?>
								</div>
								<div class="modal-footer">
									<a href="<?php echo base_url('administrator/edit_infoppdb')."/".$list->id_info_ppdb."/".$list->tgl_posting; ?>" class="btn btn-info btn-xs" title="Ubah"><span class="glyphicon glyphicon-edit"></span></a>
								</div>
							</div>
						</div>
					</div>
					
					<div class="modal fade" id="hapus<?php echo $list->id_info_ppdb; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
										<h4 class="modal-title" id="myModalLabel">Hapus "<?php echo $list->judul_info_ppdb; ?>"?</h4>
									</div>
									<div class="modal-footer">
										<a href="<?php echo base_url('administrator/hapus_infoppdb')."/".$list->id_info_ppdb; ?>" class="btn btn-danger btn-xs" title="Hapus"><span class="glyphicon glyphicon-trash"></span></a>
									</div>
								</div>
							</div>
						</div>
				...
				<hr>
				<h2><b>Input Info PPDB</b></h2>
				
				<?php require_once('tinymce.php') ?>
				<?php echo validation_errors(); ?>
				<?php echo form_open("administrator/proses_input_infoppdb", "class='form-horizontal' role='form' name='myform'"); ?>
				
				<div class="control-group">
					<label for="judul" class="control-label">Judul</label>
					<div class="controls"><?php echo form_input('judul', "", 'placeholder="" class="form-control" id="judul" /required'); ?></div>
				</div>
				<div class="control-group">
					<label for="info" class="control-label">Info</label>
					<div class="controls"><?php echo form_textarea('info', "", 'placeholder="" class="input-xxlarge" id="info"'); ?></div>
				</div>
				<br/>
