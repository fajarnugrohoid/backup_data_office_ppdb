...
?>	
	...
	<div class="col-md-9">
		<div style="width:100%; float:left; border:1px solid;">
			...
			<?php
				$i=$j;
				foreach ($list_strukturkurikulum as $list){
			?>
					<tr>
						<td><?php echo $i; ?></td>
						<td><?php echo $list->struktur_kurikulum; ?></td>
						<td><?php echo $list->nama_jurusan; ?></td>
						<td><?php echo $list->tgl_posting; ?></td>
						<td><?php echo $list->tahun_berlaku; ?></td>
						<td><a href="<?php echo base_url('administrator/edit_strukturkurikulum')."/".$list->id_struktur_kurikulum ."/". $list->tgl_posting?>" class="btn btn-info btn-xs" title="Ubah"><span class="glyphicon glyphicon-edit"></span></a> 
						<a data-toggle="modal" href="#<?php echo $list->id_struktur_kurikulum; ?>" class="btn btn-info btn-xs" title="hapus"><span class="glyphicon glyphicon-trash"></span></a></td>
					</tr>
					
					<div class="modal fade" id="<?php echo $list->id_struktur_kurikulum; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-footer">
										<a href="<?php echo base_url('administrator/hapus_strukturkurikulum')."/".$list->id_struktur_kurikulum; ?>" class="btn btn-danger btn-xs" title="Hapus"><span class="glyphicon glyphicon-trash"></span></a>
			...
			?>
				...
				<hr>
				<h2>Tambah Struktur Kurikulum</h2>
				
				<?php echo form_open_multipart("administrator/proses_input_strukturkurikulum", "class='form-horizontal' role='form'"); ?>
				<div class="control-group">
					<label class="control-label" for="file">Struktur Kurikulum</label>
					<div class="controls"><input type="file" name="file_strukturkurikulum"></input><span class="help-block">Format *.pdf, *.doc, dan *.docx</span></div>
				</div>
				<div class="control-group">
					...
					<select name="jurusan">
						<?php
						if(strcmp($this->session->flashdata('id_jurusan'),"")==0){
								foreach($jurusan_pil as $jur){
						?>
									<option value="<?php echo $jur->id_jurusan; ?>"><?php echo $jur->nama_jurusan; ?></option>
						<?php 	}
						}else{ 
								foreach($jurusan_pil as $jur){
									if($jur->id_jurusan==$this->session->flashdata('id_jurusan')){
						?>
