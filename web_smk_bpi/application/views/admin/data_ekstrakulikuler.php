...
?>	
	...
	<div class="col-md-9">
		<div style="width:100%; float:left; border:1px solid;">
			...
			<?php
				$j=$i;
				foreach ($list_ekstrakulikuler as $list){
			?>
					<tr>
						<td><?php echo $j; ?></td>
						<td><?php echo $list->nama_ekstrakulikuler; ?></td>
						<td><?php echo $list->deskripsi; ?></td>
						<td><?php echo $list->nama; ?></td>
						<td><a href="<?php echo base_url('administrator/edit_ekstrakulikuler')."/".$list->id_ekstrakulikuler; ?>" class="btn btn-info btn-xs" title="Ubah"><span class="glyphicon glyphicon-edit"></span></a> <a data-toggle="modal" href="#hapus<?php echo $list->id_ekstrakulikuler; ?>" class="btn btn-info btn-xs" title="hapus"><span class="glyphicon glyphicon-trash"></span></a></td>
					</tr>
					
					<div class="modal fade" id="hapus<?php echo $list->id_ekstrakulikuler; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
									<h4 class="modal-title" id="myModalLabel">Hapus "<?php echo $list->nama_ekstrakulikuler; ?>"?</h4>
								</div>
								<div class="modal-footer">
									<a href="<?php echo base_url('administrator/hapus_ekstrakulikuler')."/".$list->id_ekstrakulikuler; ?>" class="btn btn-danger btn-xs" title="Hapus"><span class="glyphicon glyphicon-trash"></span></a>
								</div>
							</div>
						</div>
					</div>
			...
			?>
				...
				<?php echo form_open("administrator/proses_input_ekstrakulikuler", "class='form-horizontal' role='form'"); ?>
				<div class="control-group">
					<label for="ektrakulikuler" class="control-label">Ekstrakulikuler</label>
					<div class="controls"><?php echo form_input("ekstrakulikuler", $this->session->flashdata('ekstrakulikuler'), "class='form-control' id='ekstrakulikuler' /required"); ?><span class="help-block">Isikan Ekstrakulikuler yang belum tersedia</span></div>
				</div>
				<div class="control-group">
					<label class="control-label" for="deskripsi">Deskripsi</label>
					<div class="controls"><?php echo form_textarea('deskripsi', "", 'placeholder="Masukan Deskripsi" id="deskripsi" class="input-xxlarge"'); ?></div>
				</div>
				<div class="control-group">
					<label class="control-label" for="pembina">Pembina</label>
					<div class="controls">
					<select name="pembina">
						<?php
						if(strcmp($this->session->flashdata('pembina'),"")==0){
							foreach($kepegawaian_pil as $pegawai){
							?>
							...
							foreach($kepegawaian_pil as $pegawai){
								if(strcmp($pegawai,$this->session->flashdata('pembina'))==0){
							?>
									<option value="<?php echo $pegawai->id_pegawai; ?>" selected><?php echo $pegawai->nama; ?></option>
							<?php 
								}else{
								?>
									<option value="<?php echo $pegawai->id_pegawai; ?>"><?php echo $pegawai->nama; ?></option>
