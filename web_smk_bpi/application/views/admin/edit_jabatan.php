	...
	<div class="col-md-9">
		<div style="width:100%; float:left; border:1px solid;">
			<div class="col-xs-12">
				...
				<h2><a href="<?php echo base_url('administrator/input_jabatan'); ?>" title="Kembali" class="btn btn-default"><span class="glyphicon glyphicon-chevron-left"></span></a>   Edit data Jabatan</h2>
	
				<?php echo form_open("administrator/proses_edit_jabatan", "class='form-horizontal' role='form'"); ?>
				<div class="control-group">
					<label for="nip" class="control-label">Jabatan</label>
					<?php foreach($jabatan_ed as $v_jabatan){ ?>
					<div class="controls"><?php echo form_input("jabatan", $v_jabatan->jabatan, "class='form-control' id='nip' required"); ?><span class="help-block">Isikan Jabatan yang baru</span></div>
				</div>
				<div class="control-group">
					<label for="status" class="control-label">Status</label>
					<div class="controls">
						<select name="status" id="status">
						<?php if($v_jabatan->status==1){ ?>
							<option value="1" selected>Hanya 1</option>
							<option value="0">Lebih dari 1</option>
						<?php }else{ ?>
							<option value="1">Hanya 1</option>
							<option value="0" selected>Lebih dari 1</option>
						<?php } ?>
						</select>
					</div>
				</div><?php } ?><br/>
					...
					<div class="controls">
						<select name="jajaran" id="urut">
							<?php for($urut=1;$urut<=10;$urut++){
									if(strcmp($urut,$v_jabatan->urut)==0){?>
										<option value="<?php echo $urut; ?>" selected><?php echo $urut; ?></option>
							<?php 	}else{ ?>
										<option value="<?php echo $urut; ?>"><?php echo $urut; ?></option>
							<?php	} ?>
							<?php } ?>
						</select>
					<span class="help-block">Jajaran digunakan untuk mengurutkan jabatan dari urutan teratas.</span></div>
				</div>
				
				<input type="hidden" name="id_jabatan" value="<?php echo $v_jabatan->id_jabatan; ?>">
				<br/>
