<?php
	...
	$query  = "SELECT distinct(b.id),a.code,b.type,b.quota,a.name,a.is_border,substring(a.code,1,1) as code
				...
				where (substring(a.code,1,1)=2 or substring(a.code,1,1)=3 or substring(a.code,1,1)=4) and (b.type='achievement' or b.type='rmp' or b.type='uu_guru' or b.type='inklusif')
	...
	while($passgrad != FALSE){ // selama masih ada sekolah yg harus di sorting
		...
		foreach($listsekolah as $sekolah){ // sort tiap sekolah
			...
			if($sekolah['status'] == 0){
				...
				process_sort($listsekolah[$id]['data'], 'skor_komulatif','total','skor_jarak', 'score_limit', 'range');
					...
					for($i = $jml_pend-1; $i>$sekolah['kuota']-1; $i--){
						if($listsekolah[$id]['data'][$i]['pilihan2'] != $sekolah['id']){ /* lempar ke pilihan 2 */
							...
							if ($listsekolah[$id]['jalur']=='achievement'){ /*prestasi*/
								...
								$listsekolah[$id]['data'][$i]['score_limit'] = ($listsekolah[$id]['data'][$i]['score_range2']*0.7) + ($listsekolah[$id]['data'][$i]['total2']*0.3);
							}
	...
	$result=$mysqli->query('TRUNCATE TABLE `ppdb_filtered_nonacademic`') or die(mysqli_error($mysqli));
		...
		where (substring(b.code,1,1)=2 or substring(b.code,1,1)=3 or substring(b.code,1,1)=4) and (a.type='achievement' or a.type='rmp' or b.type='uu_guru' or b.type='inklusif') ORDER BY a.id ASC";
