<?php
	...
	while($passgrad != FALSE){ // selama masih ada sekolah yg harus di sorting
		...
		foreach($listsekolah as $sekolah){ // sort tiap sekolah
			...
			if($sekolah['status'] == 0){
				...
				$luarkota=0;
				if (($listsekolah[$id]['code']!=4) || ($listsekolah[$id]['type']=='achievement')){
	...
	$result=$mysqli->query('TRUNCATE TABLE `ppdb_filtered_nonacademic`') or die(mysqli_error($mysqli));
		...
		where (substring(b.code,1,1)=2 or substring(b.code,1,1)=3 or substring(b.code,1,1)=4) and (a.type='achievement' or a.type='rmp' or a.type='mou' or a.type='uu_guru' or a.type='inklusif') ORDER BY a.id ASC";
		$rpilihan = $mysqli->query( $qpilihan) or die(mysqli_error($mysqli));
		$j=1;
		$qfiltered='';
		while($row = mysqli_fetch_array($rpilihan)){
			$passinggrade = 0.00;
			$passinggrade_foreigner = 0.00;
			$i=0;
			$count_filtered=0;
			$count_filtered_foreigner=0;
			foreach($listsekolah[$row['id']]['data'] as $r){
				$parts[] ="('" . $r['id'] . "', '" . $r['accepted_school'] . "')";
				$passinggrade = $r['score_cumulative'];
				if ($r['is_foreigner']==2){
					$passinggrade_foreigner = $r['score_cumulative'];
					$count_filtered_foreigner++;
				}
				$count_filtered++;
				if ($r['accepted_status']==2) {$i++;}
			}
			
			$update_pg = "update ppdb_statistic set passing_grade ='" . $passinggrade . "',filtered_total='" . $count_filtered . "',passing_grade_foreigner='" . $passinggrade_foreigner . "', 	filtered_foreigner='" . $count_filtered_foreigner . "'  where `option`='" . $row['id'] .  "'";
			echo 'query' . $update_pg . '<br/>';
			$rupdate=$mysqli->query($update_pg) or die(mysqli_error($mysqli));
			@mysqli_free_result($rupdate);
