...
class ViewData extends ProcessSelection
{
    ...
    public function insertToDb()
    {
        ...
        while ($row = mysqli_fetch_array($rpilihan)) {
            ...
            foreach ($this->listsekolah[$row['id']]['inner']['data'] as $r) {
                //    echo $r['id'] . '-' . $r['accepted_school'];
                $parts[]                    = "('" . $r['id'] . "', '" . $r['accepted_school'] . "', '0')";
                $passinggrade_inner_wilayah = $r['total'];
            }

            foreach ($this->listsekolah[$row['id']]['combine_area']['data'] as $r) {
            ...
            $filtered_total = count($this->listsekolah[$row['id']]['inner']['data']) + count($this->listsekolah[$row['id']]['combine_area']['data']) + count($this->listsekolah[$row['id']]['bandungraya']['data']) + count($this->listsekolah[$row['id']]['luar']['data']);
    ...
    public function showData()
    {
        ...
        foreach ($this->listsekolah as $sekolah) {
            ...
            echo '<tr><td colspan="5">Dalam Wilayah</td><td>Quota : ' . $sekolah['inner']['old_quota_dw'] . ' + ' . $sekolah['luar']['remaining_quota'] . '</td></tr>';
            $j = 0;
            ...
            echo '<tr><td colspan="5">Gabungan Wilayah</td><td>Quota : ' . $sekolah['combine_area']['old_quota_gw'] . '</td></tr>';
            $k = 0;
