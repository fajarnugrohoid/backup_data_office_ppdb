...
require __DIR__ . '/../vendor/leeoniya/dump-r/dump_r.php';
...
class ProcessSelection extends InitializationData{
    ...
    public $history_idx_dw = 0;
    public $history_idx_gw = 0;
    ...
    private function move_choice_student(&$listschool, $region_origin, $region_target, $accepted_school, $accepted_status, $school_idx, $student_idx){   
        ...
        $this->listschool[$school_idx][$region_origin]['data'][$student_idx]['accepted_school'] = $accepted_school;
            ...
            //echo 'name:' . $this->listschool[$school_idx][$region_origin]['data'][$student_idx]['name'] .'-ori:'. $region_origin .'-target:'. $region_target . '-sc:' . $accepted_school . '-sts:' . $accepted_status . '-sc_idx:'  . $school_idx . '<br/>';
    ...
    private function cut_school_under_pg(&$listschool, $region_origin, $count_students, $quota, $active_school, $school_idx, $status){
        for ($student_idx = $count_students - 1; $student_idx > $quota - 1; $student_idx--) {
            ...
            //echo 'active_school:' . $active_school .  '-name:'. $this->listschool[$school_idx][$region_origin]['data'][$student_idx]['name'] . '-' . $second_choice . '!=' . $active_school . '<br/>';
            ...
            if ($second_choice == $active_school) {
                ...
                if ( ($second_choice=='9999') || (!isset($this->listschool[$second_choice_idx]) ) ){
                    ...
                    if ( ($region_origin=='gw') && ($is_insentif2=='1') ){
                        //echo 'name_dw_secondchoice:'. $this->listschool[$school_idx][$region_origin]['data'][$student_idx]['name'] . '<br/>';
                        $region_target = 'dw';
                        ...
                        //echo 'name_gw_secondchoice:'. $this->listschool[$school_idx][$region_origin]['data'][$student_idx]['name'] . '<br/>';
                        $this->move_choice_student($this->listschool, $region_origin, $region_target, $second_choice,'2', $school_idx, $student_idx);
                        $this->listschool[$second_choice_idx][$region_target]['filtered'] = 0;
    ...
    private function merge_br_lk(&$listschool){
        ...
        for($school_idx=0;$school_idx<count($this->listschool);$school_idx++){
            ...
            echo '<br/>' . $this->history_idx . '-' . $this->listschool[$school_idx]['gw']['id'];
    ...
    private function merge_dw_gw(&$listschool){
        $status_dw = true;
        $status_gw = true;
        $temp_status_dw_gw = true;
        $temp_status_br_lk = true;

        $res_status_dw_gw = true;
        $res_status_br_lk = true;
        echo 'history_idx:' . $this->history_idx . '<br/>';
        $this->history_idx++;
        $this->history_idx_dw++;
        $this->history_idx_gw++;
        ...
        for($school_idx=0;$school_idx<count($this->listschool);$school_idx++){
            ...
            $res_status_br_lk=$this->check_filtered_br_lk($this->listschool,$school_idx, $this->history_idx, $status_dw, $status_gw);
            if ($res_status_br_lk==false){
                $temp_status_br_lk = $res_status_br_lk;
            }

            $res_status_dw_gw=$this->check_filtered_dw_gw($this->listschool,$school_idx, $this->history_idx, $this->history_idx_dw, $history_idx_gw, $status_dw, $status_gw);
            if ($res_status_dw_gw==false){
                $temp_status_dw_gw = $res_status_dw_gw;
            }


        } //end of for school loop

        if (($temp_status_dw_gw==false) || ($res_status_br_lk==false)){
            $this->merge_dw_gw($this->listschool);
        }
    }


    function check_filtered_br_lk(&$listschool,$school_idx,$history_idx, $status_br, $status_lk){
        $this->functions->sorting($this->listschool[$school_idx]['br']['data'], 'total1', 'total2', 'score_bahasa', 'score_english', 'score_math', 'score_physics', 'score_range', 'range');
            ...
            if ($count_br_lk_students < $quota){
                ...
                $this->listschool[$school_idx]['gw']['data'] = $this->historylist[$this->history_idx-1][$school_idx]['gw']['data'];
                $this->listschool[$school_idx]['dw']['filtered'] = 0;
                $this->listschool[$school_idx]['gw']['filtered'] = 0;
                $this->check_filtered_dw_gw($this->listschool,$school_idx, $this->history_idx, $this->history_idx_dw, $this->history_idx_gw, true, true);
            }
    ...
    function check_filtered_dw_gw(&$listschool,$school_idx,$history_idx, $history_idx_dw, $history_idx_gw, $status_dw, $status_gw){
        if ($this->listschool[$school_idx]['dw']['filtered']==0){
            ...
            if ($count_students > $quota) {
                for ($i = $count_students - 1; $i > $quota - 1; $i--) {
                    ...
                    $second_choice = $this->listschool[$school_idx]['dw']['data'][$i]['second_choice'];
                    $accepted_status = $this->listschool[$school_idx]['dw']['data'][$i]['accepted_status'];

                    if ($accepted_status=='1'){
                        $this->move_choice_student($this->listschool, 'dw', 'gw', $first_choice,'1', $school_idx, $i);    
                    }else{
                        $this->move_choice_student($this->listschool, 'dw', 'gw', $second_choice,'2', $school_idx, $i);    
                    }
                    $this->listschool[$school_idx]['gw']['filtered'] = 0;
                    $status_dw = false;
                } //end for
            ...
            $this->historylist[$history_idx_dw][$school_idx]['dw']['id'] = $this->listschool[$school_idx]['dw']['id'];
            $this->historylist[$history_idx_dw][$school_idx]['dw']['data'] = $this->listschool[$school_idx]['dw']['data'];
        ...
        if ($this->listschool[$school_idx]['gw']['filtered']==0){
            ...
            if ($count_students > $quota) {
                //echo 'count_students_gw:' . $active_school . ':' . $count_students . '>' . $quota . '<br/>';    
                $status_gw=$this->cut_school_under_pg($this->listschool, 'gw', $count_students, $quota, $active_school, $school_idx, $status_gw);
                
            } // end if potong siswa

            $this->historylist[$history_idx_gw][$school_idx]['gw']['id'] = $this->listschool[$school_idx]['gw']['id'];
            $this->historylist[$history_idx_gw][$school_idx]['gw']['data'] = $this->listschool[$school_idx]['gw']['data'];
        ...
        echo '<br/>' . $this->history_idx . '-' . $this->listschool[$school_idx]['gw']['id'];
        dump_r($this->historylist[$this->history_idx][$school_idx], false, true, 4, 1);
        dump_r($this->historylist[$history_idx_dw][$school_idx], false, true, 4, 1);
        dump_r($this->historylist[$history_idx_gw][$school_idx], false, true, 4, 1);
