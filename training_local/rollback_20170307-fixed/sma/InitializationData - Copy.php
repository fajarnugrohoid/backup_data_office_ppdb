...
class InitializationData extends Database
{
    ...
    public function getSchool()
    {
        ...
        FROM ppdb_option a 
        inner join ppdb_school b on (a.school = b.id) 
        where (substring(b.code,1,1)=3 ) and a.type='academic' and a.id in ('655','661','667','673','679') ORDER BY b.code, a.id";
        // $query_sch = "SELECT a.id as id,a.type,a.quota,b.name,b.is_border,substring(b.code,1,1) as code, b.foreigner_percentage 
        // FROM ppdb_option a 
        // inner join ppdb_school b on (a.school = b.id) 
        // where (substring(b.code,1,1)=3 ) and a.type='academic' ORDER BY b.code, a.id";
        ...
        $this->listsekolah[$i]['inner']['id']              = '9999';
        $this->listsekolah[$i]['inner']['name']            = 'Buangan';
        $this->listsekolah[$i]['inner']['quota']           = 0;
        $this->listsekolah[$i]['inner']['filtered']         = 1;
        $this->listsekolah[$i]['inner']['data']            = array();
        $this->listsekolah[$i]['inner']['status']          = 1;
        $this->listsekolah[$i]['inner']['remaining_quota'] = 0;

        $this->listsekolah[$i]['combine_area']['id']              = '9999';
        $this->listsekolah[$i]['combine_area']['name']            = 'Buangan';
        $this->listsekolah[$i]['combine_area']['quota']           = 0;
        $this->listsekolah[$i]['combine_area']['filtered']         = 1;
        $this->listsekolah[$i]['combine_area']['data']            = array();
        $this->listsekolah[$i]['combine_area']['status']          = 1;
        $this->listsekolah[$i]['combine_area']['remaining_quota'] = 0;
